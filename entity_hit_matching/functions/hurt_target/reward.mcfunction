...
execute if score $direct ehm.id = $direct ehm.id if score $victim ehm.id = $victim ehm.id as @e if score @s ehm.id = $victim ehm.id at @s run function #entity_hit_matching:target_is_hurt_by_player
